...
resource "google_compute_firewall" "k8s_lbfirewall" {
  ...
  name    = "kubernetes-the-hard-way-allow-lbfirewall"
  network = "${google_compute_network.k8s_network.name}"
...
resource "google_compute_target_pool" "k8s_lbtartgetpool" {
  ...
  instances = [
    "${var.zone}/${google_compute_instance.k8s_controller.0.name}",
    "${var.zone}/${google_compute_instance.k8s_controller.1.name}",
    "${var.zone}/${google_compute_instance.k8s_controller.2.name}",
  ]
